...
# Object constructor for manipulatation of a single compound at a time
class PXA:
...
#~~~~~~~~~ Ths section contains the main function ~~~~~~~~~# 

def main():
#STEP 1: Create a dataframe with the chemical formulas of the alkanes (carbon chain 10-to-25)
    df=pd.DataFrame({'Formula': []})
    numC=10
    numH=2*numC+2
    for row in range(0,16):
        df.loc[row,"Formula"]= f"C{numC}H{numH}"
        numC+=1
        numH=2*numC+2

    #STEP 2: Generate all the possible linear congeners SMILES
    congeners=generate_Congeners_From_Dataframe(df)
    congeners_dataframe=pd.DataFrame(congeners)
    congeners_dataframe.columns = ["SMILE","Group","Formula","Cl","F","Br","C","H","Halo"]
#     congeners_dataframe.to_csv("Your-File.csv", index=False)    #-------------If wanted to save the dataframe as a CSV file

    #STEP 3: Add a carboxylic acid to each congener created above
    congeners_carboxylic_dataframe= all_generateWithCarboxylic(congeners_dataframe)
    #congeners_carboxylic_dataframe.to_csv("Your-File-Carboxylic.csv", index=False)    -------------If wanted to save the dataframe as a CSV file

    #STEP 4: Generate all linear isomers of a compound
    compound_isomers_dataframe=getIsomers(compound_formula='C10H18Cl4')
    #compound_isomers_dataframe.to_csv("Your-File-Isomers.csv", index=False)    -------------If wanted to save the dataframe as a CSV file

    #STEP 5: Generate the partitioning space, Venn Diagram and Histogram 
    #~~~~~Visualized in a Jupyter Notebook
    ##df_congeners=pd.read_csv('Your-File.csv')
    ##generateVennDiagram(df_congeners)
    ##generateHistogram(df_congeners)
    ##generate_Partitioning_Space(df_congeners)
